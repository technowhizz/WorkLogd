...
import { localTimeOf } from './utils/dates';
...
test('test that start time picker works in create modal', async ({ page }) => {
    ...
    const createBody = await createResponse.json();
    // The picker sets a local wall-clock time and the API stores the UTC instant, so compare in
    // local terms - matching '09:30' inside the stored string only holds at UTC+0.
    expect(localTimeOf(new Date(createBody.data.start))).toBe('09:30');
});
